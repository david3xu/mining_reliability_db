...
def create_question_category_card(
    category_id: str, category_name: str, questions: List[Dict], show_details: bool = False
) -> dbc.Card:
        ...
        html.Div(
            [
                ...
                html.Div(
                    [
                        html.Span(
                            f"{question_count} Questions", className="badge bg-light text-dark me-2"
                        ),
                        html.Span("Pending Analysis", className="badge bg-warning text-dark"),
                    ]
                ),
            ],
...
def create_stakeholder_questions_overview(questions_data: Dict) -> html.Div:
    ...
    high_priority_count = sum(
        1 for cat in categories if cat.get("category_id") in high_priority_cats
    )
    ...
    metrics_row = dbc.Row(
        [
            ...
            dbc.Col([create_metric_card("0", "Completed", "Analysis progress")], width=3),
        ],
        className="mb-4",
    )
    ...
    categories_section = html.Div(
        [html.H3("Question Categories", className="mb-3"), create_questions_grid(categories)]
    )

    return create_standard_layout(
        title="Stakeholder Questions Overview", content_cards=[metrics_row, categories_section]
    )
...
def create_question_detail_view(category_data: Dict) -> html.Div:
        ...
        question_card = dbc.Card(
            [
                dbc.CardHeader(
                    [
                        html.H5(
                            f"Question {question.get('question_id', f'Q{i}')}",
                ...
                dbc.CardBody(
                    [
                        html.P(question.get("question_text", ""), className="lead mb-3"),
                        html.Div(
                            [
                        ...
                        html.Div(
                            [
                                dbc.Button(
                                    "Begin Analysis", color="primary", size="sm", className="me-2"
                                ),
                                dbc.Button(
                                    "Add Notes",
    ...
    return create_standard_layout(title=category_name, content_cards=question_cards)
        ...
        "categories_by_priority": {
            "high": len(
                [
                    ...
                    in ["tech_impact", "operational_trends", "workflow_understanding"]
                ]
            ),
            "medium": len(
                [
                    ...
                    for c in categories
                    if c.get("category_id")
                    in ["record_structure", "category_classification", "field_relationships"]
                ]
            ),
