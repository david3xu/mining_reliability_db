...
def create_workflow_metrics() -> html.Div:
    ...
    try:
        ...
        metrics = [
            ...
            create_metric_card(mapping_data.get("total_fields", 0), "Mapped Fields", True, "/workflow-process"),
...
def create_mapping_table() -> any:
    ...
    try:
        ...
        table_data = [{
            "Entity": m["entity"], "Target Field": m["target_field"],
            "Source Field": m["source_field"], "Category": m["category"],
            "Critical": "Yes" if m["critical"] else "No"
        } for m in mapping_data["mappings"]]

        return create_data_table(table_data, ["Entity", "Target Field", "Source Field", "Category", "Critical"])
    except Exception as e:
...
def create_workflow_analysis_layout() -> html.Div:
    ...
    try:
        ...
        return html.Div([
            ...
            html.Div([
                html.P("Click cards to explore detailed workflow analysis",
                      className="text-center text-muted my-4")
            ]),
            dbc.Row([
                dbc.Col([create_entity_distribution_chart()], md=12)
            ])
...
def create_workflow_process_page() -> html.Div:
    ...
    try:
        return html.Div([
            dbc.Container([
                dbc.Button("← Back to Workflow", href="/workflow", color="secondary", size="sm", className="mb-3"),
