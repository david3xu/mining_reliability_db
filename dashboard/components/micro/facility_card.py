...
def create_facility_summary_card(facility_data: dict) -> dbc.Card:
    ...
    return dbc.Card([
        dbc.CardBody([
            ...
            html.H3(f"{total_records:,}", style={"color": config.get("primary_color", "#4A90E2")}),
            html.P("Total Records", className="text-muted"),
            html.Span("Active" if active_status else "Inactive",
                     style={"padding": "4px 8px", "borderRadius": "4px",
                           "backgroundColor": status_color, "color": "#FFFFFF", "fontSize": "12px"})
        ])
