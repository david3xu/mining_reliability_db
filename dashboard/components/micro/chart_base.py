...
def create_pie_chart(labels: list, values: list, title: str) -> dcc.Graph:
    ...
    fig.update_layout(
        title=title, height=config.get("height", 400),
        font={"family": config.get("font_family", "Arial"), "size": 14},
        paper_bgcolor=config.get("background", "#FFFFFF")
    )
...
def create_bar_chart(x_data: list, y_data: list, title: str) -> dcc.Graph:
    ...
    fig = go.Figure(data=[go.Bar(x=x_data, y=y_data, marker_color=config.get("colors", ["#4A90E2"])[0])])
    fig.update_layout(
        title=title, height=config.get("height", 400),
        font={"family": config.get("font_family", "Arial"), "size": 14},
        paper_bgcolor=config.get("background", "#FFFFFF")
