...
class DashboardValidator:
        ...
        for py_file in components_path.glob("*.py"):
            ...
            for line_num, line in enumerate(content.split('\n'), 1):
                ...
                for color in matches:
                    ...
                    if ("get_dashboard_styling_config" in line or
                        "styling_config.get" in line or
                        "colors[" in line or
    ...
    def scan_direct_imports(self) -> List[Dict]:
        ...
        for py_file in components_path.glob("*.py"):
            ...
            for line_num, line in enumerate(content.split('\n'), 1):
                if (line.strip().startswith("from mine_core") and
                    "adapters" not in line and
                    "shared.common" not in line):  # shared.common is allowed for error handling
    ...
    def calculate_compliance_score(self) -> float:
        ...
        total_files = len(list((self.dashboard_path / "components").glob("*.py"))) - 1  # Exclude __init__.py
        ...
        compliant_files = sum(1 for file in self.results["adapter_compliance"]
        ...
        config_files = sum(1 for file in self.results["configuration_usage"]
                          if file["config_driven"] and file["file"] != "dashboard/components/__init__.py")
        config_score = (config_files / max(total_files, 1)) * 30

        # Calculate final score
        base_score = 100
        final_score = max(0, base_score - hardcoded_penalty - direct_imports_penalty + adapter_score + config_score - 70)
    ...
    def generate_report(self) -> str:
        ...
        status = "✅ EXCELLENT" if score >= 95 else "✅ GOOD" if score >= 85 else "⚠️ NEEDS IMPROVEMENT" if score >= 70 else "❌ POOR"
        ...
        total_component_files = len([f for f in self.results["adapter_compliance"] if f["file"] != "dashboard/components/__init__.py"])
        report.append(f"Adapter Pattern: {len(compliant_files)}/{total_component_files} files compliant")

        # Direct imports check
        if not self.results["direct_mine_core_imports"]:
            report.append("✅ No direct mine_core imports found")
        else:
            report.append(f"❌ {len(self.results['direct_mine_core_imports'])} direct mine_core imports found")
        ...
        if not self.results["hardcoded_colors"]:
            ...
            report.append(f"⚠️ {len(self.results['hardcoded_colors'])} hardcoded color values found")

        config_driven_files = [f for f in self.results["configuration_usage"] if f["config_driven"]]
        report.append(f"Configuration-driven: {len(config_driven_files)}/{total_component_files} files")
