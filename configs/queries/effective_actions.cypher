...
WITH ar, p, rc, ap, v,
     ...
     100 AS base_score,
     CASE
         WHEN v.action_plan_eval_comment IS NOT NULL AND size(v.action_plan_eval_comment) > 30 THEN 30
         WHEN v.action_plan_eval_comment IS NOT NULL THEN 15
         ELSE 5 END AS rationale_score,
     CASE
         WHEN rc.objective_evidence IS NOT NULL AND size(rc.objective_evidence) > 20 THEN 20
...
RETURN ar.action_request_number AS incident_id,
       ...
       CASE
           ...
           ELSE "Limited" END AS evidence_quality,
       CASE
           WHEN equipment_matches > 0 THEN "Equipment Pattern"
           ELSE "Direct Match" END AS discovery_method
